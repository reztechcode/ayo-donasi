...
@section('content')
        ...
        <div class="mb-11 flex justify-center mt-7">
            <div class="relative lg:w-5/12 w-full px-3" id="input">
                <input placeholder="Search..." name="q"
                    class="block w-full text-sm h-[50px] px-4 text-slate-900 bg-white rounded-full border-2 border-slate-400 appearance-none focus:border-transparent focus:outline focus:outline-2 focus:outline-info focus:ring-0 hover:border-brand-500-secondary- peer invalid:border-error-500 invalid:focus:border-error-500 overflow-ellipsis overflow-hidden text-nowrap pr-[48px]"
    ...
    <div class="px-3">
        @if ($search && !$campaigns->isEmpty())
            Menampilkan Pencarian Untuk: <b>{{ $search }}</b>
        @endif

        @if ($campaigns->isEmpty())
            <h1 class="text-center text-red-600">Mohon maaf, Tidak ada pencarian untuk: <b>{{ $search }}</b></h1>
        @endif
    </div>
